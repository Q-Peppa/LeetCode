...
class pri_que {
	...
	constructor(compare, _values) {
		if (typeof compare !== 'function') {
			throw new Error('pri_que constructor expects a compare function');
		}
	...
	static fromArray(values, compare) {
		return new pri_que(compare, values);
	}
}
...
var totalCost = (costs, k, candidates) => {
	...
	let left = -1;
	while (lq.size() < candidates) {
		lq.push(costs[++left]);
	}
	let right = costs.length;
	while (rq.size() < candidates) {
		rq.push(costs[--right]);
		if (right <= left) break;
	}
	let res = 0;
	while (k) {
		if (lq.size() * rq.size() === 0) break;
		if (lq.front() <= rq.front()) {
			console.log('v', lq.front());
			res += lq.pop();
			if (left < right) {
				lq.push(costs[++left]);
			}
		} else {
			console.log('v', rq.front());
			res += rq.pop();
